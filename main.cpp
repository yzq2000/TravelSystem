...
vector<Edge> edge[N * DAY]; //��һ�����в��24���� 

struct P{
    int fa, t;
    string wr;//where, ״̬ 
};
P p[N * DAY][N * DAY];//p[i][j]��ʾi����jʱ�̵ĸ��׽ڵ㣬ʱ���Լ�״̬ 

double dp[N * DAY][N * DAY]; //dp[i][j]��ʾi����jʱ�̵���С����ֵ 
...
void solve(string name, int st_city, int ed_range, int max_t){
    ...
    double ans_r;
    q.push({st_city, 0, 0});
    while(!q.empty()){//���Ż���priority_queue�� ��dijkstra 
        int u = q.top().ct, t = q.top().t;
        double r = q.top().r;
        q.pop();
        if(u >= ed_range * DAY && u < ed_range * DAY + DAY) {// �յ㷶Χ 
            flag = true;
            ed_city = u, ed_time = t;
            ans_r = r;
            break;
...
void Init(){
    ...
    for(int i = 1; i <= n; i++){//��һ�����л���Ϊ24��ʱ�̶�Ӧ��24���㣬ʹÿ���㶼����ʱ������ 
        for(int j = 0; j < DAY; j++){//�������ΪNode���� Node/DAY Ϊ���б�ţ�Node%DAYΪʱ�� 
            add_edge(i * DAY + j, i * DAY + (j + 1) % DAY, 1, city[i].name, city[i].risk);
        }
    }
    read(cnt_flight, risk_flight, "flight");//���뺽��ʱ�̱� 
    read(cnt_train, risk_train, "train");//�����г�ʱ�̱� 
    read(cnt_bus, risk_bus, "bus");//�����ʿʱ�̱� 
}
void read(int &cnt, int R, string wr){ 
    scanf("%*s%*s%*s%*s%*s");
